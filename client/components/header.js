  ...
  return (
    <div>
      <nav className="flex items-center justify-between flex-wrap bg-black p-6">
        <div className="flex items-center flex-shrink-0 text-pink-900 mr-6">
          <span className="font-semibold text-pink-900 tracking-tight uppercase">{userName}</span>
          <span className="font-semibold mx-4 text-pink-900 tracking-tight">
        ...
        <div className="w-full block justify-end flex-grow lg:flex lg:items-center lg:w-auto">
          <div>
            <Link
              to="/"
              className="inline-block text-sm justify px-4 py-2 leading-none border rounded text-pink-600 border-pink-600  hover:border-transparent hover:text-teal-500 hover:bg-pink-800 m-4 lg:mt-0"
            >
              Back to main
            </Link>
            <div>
            ...
            {repositoryName && (
              <Link
                to={`/${userName}`}
                className="inline-block text-sm px-4 py-2 leading-none border rounded text-pink-600 border-pink-600 hover:border-transparent hover:text-teal-500 hover:bg-pink-800 mt-4 lg:mt-0"
              >
                Back to repo
              </Link>
            )}
      ...
      <Route
        exact
        path="/:userName"
        component={() => <RepoList  searchedName={searchedName} />}
      />
    </div>
  )
}
